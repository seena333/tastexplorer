        ...
        <div class="container contact_us_page my-5">
            <div class="row">
                <div class="col-lg-12 px-lg-5 mb-4">
                    <h1 class="fw-bold"> Choice Food Here :-</h1>
                </div>
                <div class="col-lg-7 order-lg-1 order-2 col-md-12 px-lg-5 bg-white rounded-3">

<?php $dsql=mysqli_query($con,"SELECT  * FROM items where group1='$food' ");?>
     

                   
					
<table>
    <?php while($d_arr=mysqli_fetch_assoc($dsql))	
			{  ?>
<tr>
 <form class="row mb-3" method="POST" action="http://localhost/tast_expo/cust_loc.php">     
<input type="hidden"   name="fid" style="text-align:center;font-size: 15;font-weight: bold;"; value=<?php print("$d_arr[fid]");?>>


						  <td>		<div class="col-md-6 p-3">
                           <?php  print" <img src='$d_arr[fimage]' width='150' height='150' border='0' alt=''>  ";?> 
                          </div></td>

	<td>		<div class="col-md-6 p-3">
                           <?php  print"<input class='btn px-4 py-3 btn-outline-success' type='submit' name='item' value= '$d_arr[fname]'";?> >
                          </div></td>
    </form>
</tr>
<?php
