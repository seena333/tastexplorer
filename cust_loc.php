        ...
        <div class="container contact_us_page my-5">
            <div class="row">
                <div class="col-lg-12 px-lg-5 mb-4">
                    <h1 class="fw-bold">Select the Location Here:-</h1>
                </div>
                <div class="col-lg-7 order-lg-1 order-2 col-md-12 px-lg-5 bg-white rounded-3">

                   

                        <input type="hidden" name="_token" value="fYpvwC9r64pptNrUjbVuAKMy6dHbWzF3rLGcMxHs" autocomplete="off">                      
						<div class="col-md-6 p-3">
                            <input required=""  disabled placeholder=" Near By Location   : "  type="text"  id="">
                         </div>  
                   <div class="col-md-6 p-3">
						<form  method="POST" enctype="multipart/form-data"  action="http://localhost/tast_expo/cust_hotel.php">
     <?php print"<input type='hidden'   name='fid'  value='$fid' >"; ?>
	<div class="text-end mt-4">
	<?php	
                  
			 $dos=mysqli_query($con,"select distinct (lname) as 'lname'  from hotels ")  or die(mysqli_query());	
 			print("<select class='input--style-1' style='border-color: #ffffff;width: 100%';  name='loc'  style='width: 100%; text-align: left; font: 0.95em sans-serif;'>");
            while($d_arr=mysqli_fetch_assoc($dos))	
			{
			print("<option  value='$d_arr[lname]'>$d_arr[lname] ");
			}
			echo"</select>  ";		
			?>
      </div>
                        ...
                        <div class="text-end mt-4">
                            <input class="btn px-4 py-3 btn-outline-success" type="submit" name='Cust_Loc' value="Submit" >
                        </div>
                    </form>
                </div>
